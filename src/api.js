...
function Api(){
  ...
  function locations(search){
    let url = 'http://localhost:3000/locations/' + search;
    return send_request(url).then(res => res).catch(err => { throw(err) })
  }

  function location_details(placeid){
    let url = 'http://localhost:3000/locations/details/' + placeid;
    return send_request(url).then(res => res).catch(err => { throw(err) })
  }

  function weather(latlng){
    let url = 'http://localhost:3000/weather/' + latlng;
    return send_request(url).then(res => res).catch(err => { throw(err) })
  }
